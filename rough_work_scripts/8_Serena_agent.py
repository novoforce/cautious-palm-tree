...
def streamlit_app():
    ...
    for message in st.session_state.messages:
        with st.chat_message(message["role"]):
            ...
            # ### FIXED: If the message is from the assistant and has artifacts,
            # ### put them in a collapsible expander.
            if message["role"] == "assistant" and "artifacts" in message and message["artifacts"]:
                with st.expander("🖼️ View Generated Content", expanded=False): # Older messages are collapsed
                    image_artifacts = [
                        (filename, part)
        ...
        with st.chat_message("assistant"):
            with st.spinner("🧠 The agent is thinking..."):
                try:
                    ...
                    # ### FIXED: If there are new artifacts, display them in a
                    # ### collapsible expander that is OPEN by default.
                    if artifacts:
                        with st.expander("🖼️ View Generated Content", expanded=True): # New content is expanded
                            image_artifacts = [
                                (filename, part)
                    ...
                    # Use st.rerun() to immediately redraw the history with the new message
                    st.rerun()

# agent.py # Full runnable code for the StoryFlowAgent example
import logging
import asyncio
...
# --- Function to Interact with the Agent (Corrected) ---
async def call_agent(runner: Runner, session_service: InMemorySessionService, user_input_topic: str):
    ...
    # Correct way to handle an async generator: iterate with `async for`
...
async def main():
    """Sets up and runs the agent workflow."""
    ...
    await call_agent(runner, session_service, "a lonely robot finding a friend in a junkyard")

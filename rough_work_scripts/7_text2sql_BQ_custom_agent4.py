...
try:
    ...
    st.error(
        "Google ADK components not found. Please ensure 'google-adk' is installed."
    )
    ADK_AVAILABLE = False
    # Define dummy classes if ADK is not available to avoid NameError
    SequentialAgent = LlmAgent = InMemorySessionService = Runner = (
        BuiltInCodeExecutor
    ) = object
...
try:
    ...
    st.error(
        "Google GenAI types not found. Please ensure 'google-generativeai' is installed."
    )
...
class CodeWriterOutput(BaseModel):
    ...
    generated_code: str = Field(
        description="The Python code generated based on the user's request. Must be a valid Python code string."
    )
...
class CodeReviewerOutput(BaseModel):
    ...
    review_comments: str = Field(
        description="Constructive feedback and review comments for the provided code, formatted as bullet points in a single string."
    )
...
class CodeRefactorerOutput(BaseModel):
    ...
    refactored_code: str = Field(
        description="The final, refactored Python code, based on the original code and review comments. Must be a valid Python code string."
    )
...
async def run_debug_pipeline_async(user_query: str, current_session_id: str):
    ...
    return {
        "generated_code": get_attribute_safely(
            writer_obj, "generated_code", "generated_code"
        ),
        "review_comments": get_attribute_safely(
            reviewer_obj, "review_comments", "review_comments"
        ),
        "refactored_code": get_attribute_safely(
            refactorer_obj, "refactored_code", "refactored_code"
        ),
...
st.markdown(
    "Enter a description -> **Write Code** -> **Review Code** -> **Refactor Code**"
)
...
with col1:
    ...
    if st.button("⚙️ Run Code Refinement Pipeline", disabled=not ADK_AVAILABLE):
        if user_query and ADK_AVAILABLE and GENAI_TYPES_AVAILABLE:
            ...
            current_session_id = (
                f"{SESSION_ID_PREFIX}{st.session_state.session_counter}"
            )
            ...
            try:
                with st.spinner("🤖 Running AI pipeline with Pydantic validation..."):
                    ...
                    results_data = asyncio.run(
                        run_debug_pipeline_async(user_query, current_session_id)
                    )
                    st.session_state.results = results_data

                    if results_data and (
                        results_data.get("generated_code")
                        or results_data.get("refactored_code")
                    ):
                        status_placeholder.success("✅ Pipeline finished successfully!")
                    else:
                        status_placeholder.warning(
                            "⚠️ Pipeline finished, but some results might be missing."
                        )
...
with col2:
    ...
    if "results" in st.session_state and st.session_state.results:
        ...
        with st.expander(
            "🖋️ Step 1: Initial Code Generation (CodeWriterAgent)", expanded=True
        ):
            generated_code = clean_code_output(results.get("generated_code"))
            if generated_code:
                st.code(generated_code, language="python")
            else:
                st.warning("No code generated or retrieved.")

        with st.expander("🔍 Step 2: Code Review (CodeReviewerAgent)", expanded=True):
            review_comments = results.get("review_comments")
            if review_comments:
                st.markdown(review_comments)
            else:
                st.warning("No review comments generated or retrieved.")

        with st.expander(
            "🔧 Step 3: Refactored Code (CodeRefactorerAgent)", expanded=True
        ):
            refactored_code_raw = results.get("refactored_code")
            refactored_code_cleaned = clean_code_output(refactored_code_raw)
            if refactored_code_cleaned:
                st.code(refactored_code_cleaned, language="python")
            else:
                st.warning("No refactored code generated or retrieved.")
        ...
        if final_message:
            ...
            st.warning("No final message captured from the pipeline.")

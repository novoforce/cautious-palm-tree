...
###<<<<<<<<<<<<<<<<<<<<<<<<<<<ASHISH working version without audio and only text<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<###
# from typing import AsyncIterable, Dict, Any
# async def agent_to_client_messaging(
#     websocket: WebSocket, live_events: AsyncIterable[Event | None]
# ):
...
#             # --- ENABLE FOR VERY DETAILED RAW ADK EVENT LOGGING ---
#             # logger.error(f"[ADK_RAW_EVENT]: {event}")

#             # 1. Handle Turn Completion or Interruption
#             if event.turn_complete or event.interrupted:
#                 message: Dict[str, Any] = { # Type hint for clarity
#                     "turn_complete": event.turn_complete,
#                     "interrupted": event.interrupted,
#                 }
...
#                         "mime_type": "text/plain", # Or a specific mime_type for tool calls
#                         "data": f"Tool Call: {tool_name}, Args: {arguments}",
#                         "role": "system", # Or "tool_call" for client to handle differently
#                     }
...
#                 content_role = event.content.role # 'user' or 'model'
#                 # logger.info(f"[ADK_EVENT_CONTENT] Role: '{content_role}', Partial: {event.partial}, #Parts: {len(event.content.parts)}")
                
#                 for i, part in enumerate(event.content.parts):
#                     # --- CRITICAL LOGGING FOR PART INSPECTION ---
#                     logger.error(f"[ADK_EVENT_PART_{i}] Role: '{content_role}', Type: {type(part)}, Details: {part}")

#                     if not isinstance(part, types.Part):
#                         logger.warning(f"Part {i} is not an instance of types.Part. Skipping.")
#                         continue

#                     # Initialize messages to be sent for this part
#                     text_message_to_send: Dict[str, Any] | None = None
#                     audio_message_to_send: Dict[str, Any] | None = None

#                     # A. Check for Text in the part
#                     if part.text:
#                         if content_role == "user": # Transcription of user's speech
#                             text_message_to_send = {
#                                 "mime_type": "text/plain",
#                                 "data": part.text,
#                                 "role": "user_transcription",
#                             }
#                         elif content_role == "model": # Text from the agent/model
#                             text_message_to_send = {
#                                 "mime_type": "text/plain",
#                                 "data": part.text,
#                                 "role": "model",
...
#                     # B. Check for Inline Audio Data in the part (expected for model's audio output)
#                     if part.inline_data and \
#                        part.inline_data.mime_type and \
#                        part.inline_data.mime_type.startswith("audio/pcm"):
                        
#                         if content_role == "model": # Only process model's audio
...
#                                     "role": "model",
#                                 }
#                                 logger.info(f"Prepared audio message for model from part {i}")
#                             else:
#                                 logger.warning(f"Part {i} (model audio) has empty audio data.")
#                         # else:
#                         # logger.debug(f"Part {i} has audio/pcm but role is '{content_role}', not 'model'. Skipping audio send.")
...
#                         logger.error(f"[AGENT_TO_CLIENT_SEND - AUDIO_PART]: Role '{audio_message_to_send['role']}', Mime: '{audio_message_to_send['mime_type']}'") # Keep as ERROR for visibility
...
#<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<ASHISH working with audio and text>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
# async def agent_to_client_messaging(
#     websocket: WebSocket, live_events: AsyncIterable[Event | None]
# ):
#     """Agent to client communication. Processes events from ADK and sends to WebSocket client."""
...
#             # logger.error(f"[ADK_RAW_EVENT]: {event}") # Keep for deep debugging if needed
...
#             if event.content and event.content.parts:
#                 # Get the role from the event's content object. This role applies to all parts within this content.
#                 event_content_role = event.content.role 
#                 # logger.info(f"[ADK_EVENT_CONTENT_OBJECT] Role: '{event_content_role}', Partial: {event.partial}, #Parts: {len(event.content.parts)}")
                
#                 for i, part in enumerate(event.content.parts):
#                     # Log the role associated with the event.content for context, and then the part itself
#                     logger.error(f"[ADK_EVENT_PART_{i}] EventContentRole: '{event_content_role}', PartType: {type(part)}, PartDetails: {part}")
...
#                     # A. Check for Text in the part
#                     if part.text:
#                         if event_content_role == "user": 
#                             text_message_to_send = {
#                                 "mime_type": "text/plain",
#                                 "data": part.text,
#                                 "role": "user_transcription",
#                             }
#                         elif event_content_role == "model": 
#                             text_message_to_send = {
#                                 "mime_type": "text/plain",
#                                 "data": part.text,
#                                 "role": "model",
#                             }
#                         # If event_content_role is None but part has text, it's ambiguous.
#                         # For now, we only send text if event_content_role is 'user' or 'model'.
#                         # You might need to adjust this if text appears with event_content_role == None.
                    
#                     # B. Check for Inline Audio Data in the part
#                     if part.inline_data and \
#                        part.inline_data.mime_type and \
#                        part.inline_data.mime_type.startswith("audio/pcm"):
                        
#                         # If it's audio/pcm, we assume it's from the model if not explicitly user.
#                         # The user's audio is sent via client_to_agent_messaging and processed by ADK,
#                         # resulting in user_transcription text, not raw audio back through here.
#                         # Therefore, PCM audio here is highly likely to be the model's speech output.
#                         # The event_content_role might be 'model' OR 'None' as per logs.
                        
#                         # --- MODIFIED CONDITION ---
#                         # if event_content_role == "model" or event_content_role is None:
#                         # Let's be more direct: if it's audio/pcm from the event stream, it's model output
#                         # unless explicitly tagged otherwise (which isn't happening for user audio input here).
#                         # The key is that `input_audio_transcription` results in TEXT parts with role 'user'.
#                         # `output_audio_transcription` results in TEXT parts with role 'model' AND audio parts.
#                         # The audio parts themselves might not carry the 'model' role directly on event.content.role
#                         # if they are streamed separately from the main text content.

#                         audio_data_bytes = part.inline_data.data
#                         if audio_data_bytes:
#                             audio_message_to_send = {
#                                 "mime_type": "audio/pcm", # The client expects this for playback
#                                 "data": base64.b64encode(audio_data_bytes).decode("ascii"),
#                                 "role": "model", # Critically, tag it as 'model' for the client
#                             }
...
#                         # --- END OF MODIFIED CONDITION BLOCK ---
...
#                     if audio_message_to_send:
#                         # Ensure this log remains ERROR for high visibility during testing audio
#                         logger.error(f"[AGENT_TO_CLIENT_SEND - AUDIO_PART]: Role '{audio_message_to_send['role']}', Mime: '{audio_message_to_send['mime_type']}'") 
#                         await websocket.send_text(json.dumps(audio_message_to_send))
